...
const debugAdapterLogWriter = fs.createWriteStream(path.resolve(logBasePath, "DebugAdapter.log"));
...
function waitForSessionFile(triesRemaining) {
    ...
    if (triesRemaining > 0) {
        if (utils.checkIfFileExists(debugSessionFilePath)) {
            ...
            setTimeout(() => waitForSessionFile(triesRemaining - 1), 1000);
        }
    }
    else {
        ...
        process.stdout.write(`Content-Length: ${Buffer.byteLength(errorJson, "utf8")}\r\n\r\n${errorJson}`, "utf8");
    }
}
